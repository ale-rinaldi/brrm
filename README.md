...
- **Limite DST (cambio ora legale):** stessa radice del limite cross-midnight — siccome partenze e arrivi sono memorizzati come time-of-day, brrm non sa che il fuso e' cambiato tra i due eventi. Se una gara attraversa la transizione DST (ultima domenica di marzo `02:00 CET → 03:00 CEST`, oppure ultima domenica di ottobre `03:00 CEST → 02:00 CET`), il tempo calcolato e' sbagliato di +1h (spring forward) o +23h (fall back, perche' scatta la regola cross-midnight). Per lo stesso motivo l'`unix_ms` di partenza scritto su CSV/Sheets usa l'offset POSIX corrente (`System.TimeZone` al momento di `Now()`), quindi e' shiftato di un'ora rispetto al reale se la partenza era nel fuso opposto. **Nessuna gestione automatica:** in pratica le gare Soap Box non corrono mai alle 2-3 di notte di ultima domenica di marzo/ottobre, quindi il caso e' irrilevante. La soluzione strutturale richiederebbe di memorizzare unix-ms (UTC) invece di time-of-day.
